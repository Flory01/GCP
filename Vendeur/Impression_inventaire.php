<?php
    ...
    $Jour = !empty($_GET["jour"]) ? $_GET["jour"] : strftime("%d");
    $Mois = !empty($_GET["mois"]) ? $_GET["mois"] : strftime("%m");
    $Annee = !empty($_GET["annee"]) ? $_GET["annee"] : strftime("%Y");

    $requete_sel_paie = $connexion->prepare("SELECT * FROM `paiement` AS `P` INNER JOIN `agent` AS `A` ON A.PK_Agent = P.Agent INNER JOIN `commande` AS `C` ON C.PK_Cmd = P.Commande INNER JOIN temps AS `T` ON T.PK_Temps = C.Date_Cmd INNER JOIN `client` AS `Cl` ON P.Client = Cl.PK_Cl INNER JOIN `genre` AS `G` ON G.PK_Genre = A.Genre_Agent WHERE P.Observation='PAYE' AND T.Jour = '$Jour' AND T.Mois = '$Mois' AND T.Annee = '$Annee' AND P.Agent = $id_agent");
        //Exécution de la requête
    $requete_sel_paie->execute();

    if ($rs_sel_paie = $requete_sel_paie->fetch(PDO::FETCH_ASSOC)) {
    	$nom_agent = $rs_sel_paie["Nom_Agent"]." ".$rs_sel_paie["PostNom_Agent"];
    	$genre = ($rs_sel_paie["Lib_Genre"] === "M") ? "M." : "Mme" ;
	    ...
	    $pdf->AddPage();
	        ...
	        //Titre du tableau
	        $Titre = utf8_decode("RAPPORT JOURNALIER DE VENTE DU : ").$Jour."-".$Mois."-".$Annee;
	        ...
	        $pdf->Cell(239/*Largeur*/, 10/*hauteur*/, utf8_decode("Rapport journalier établi par : ".$genre." ".$nom_agent), ''/*Encadrement*/, 1, 'L');

	        $rq_sel_cmd = $connexion->prepare("SELECT * FROM `commande` AS `C` INNER JOIN `paiement` AS `P` ON P.Commande = C.PK_Cmd INNER JOIN `temps` AS `T` ON T.PK_Temps = C.Date_Cmd WHERE P.Observation='PAYE' AND T.Jour = '$Jour' AND T.Mois = '$Mois' AND T.Annee = '$Annee' AND P.Agent = $id_agent");
		    //Exécution de la requête
		    $rq_sel_cmd->execute();

		    $total_cmd = $total_vente = $total_qte_vente = $pt_ref = 0;
		    ...
		    while ($rs_sel_cmd = $rq_sel_cmd->fetch(PDO::FETCH_ASSOC)) {
			    ...
			    while ($rs_sel_ref = $rq_sel_ref->fetch(PDO::FETCH_ASSOC)) {
			    	...
			    	$tab_id_cmd[$rs_sel_cmd["PK_Cmd"]]["PU"][$rs_sel_ref["PK_Ref"]] = $rs_sel_ref["P_U"]." FC";
			        ...
			        $pdf->Cell(50,10,$tab_id_cmd[$rs_sel_cmd["PK_Cmd"]]["PU"][$rs_sel_ref["PK_Ref"]],'TBLR',0,'C');
			    ...
			    $total_vente += $tab_id_cmd[$rs_sel_cmd["PK_Cmd"]]["PTC"];
			    $total_qte_vente += $tab_id_cmd[$rs_sel_cmd["PK_Cmd"]]["QTC"];
		    }

		    $pdf->SetTextColor(52,73,94);
		    $pdf->SetTextColor(255,255,255);
		    $pdf->Cell(89,10,"TOTAL VENTE",'TBLR',0,'C',true);
			$pdf->Cell(50,10,$total_qte_vente,'TBLR',0,'C',true);
			$pdf->Cell(100,10,$total_vente." FC",'TBLR',1,'C',true);
			$pdf->Ln(10);
	    ...
	    $pdf->Output("I"/*Mode d'affichage*/, 'Rap_jour_du_'.$Jour."-".$Mois."-".$Annee.'.pdf'/*Nom du pdf*/,true/*Prise en charge de l'encodage UTF8*/);
	}else{
        header("refresh:1380; url=../../rien.php");
    }
?>