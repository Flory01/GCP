<?php
    ...
    $pdf->AddPage();
        ...
        //Titre du tableau
        $Titre = utf8_decode("LISTE DES COMMANDES EN ATTENTE DE VALIDATION");
        ...
        $rq_sel_cmd = $connexion->prepare("SELECT * FROM `commande` AS `C` INNER JOIN `paiement` AS `P` ON P.commande = C.PK_Cmd INNER JOIN `temps` AS `T` ON T.PK_Temps = C.Date_Cmd WHERE P.Observation='NON PAYE' AND C.livraison = 'NON'");
        ...
        while ($rs_sel_cmd = $rq_sel_cmd->fetch(PDO::FETCH_ASSOC)) {
            ...
            while ($rs_sel_ref = $rq_sel_ref->fetch(PDO::FETCH_ASSOC)) {
                ...
                $pdf->Cell(50,10,$tab_id_cmd[$rs_sel_cmd["PK_Cmd"]]["produit"][$rs_sel_ref["PK_Ref"]],'TBLR',0,'C');
    ...
    $pdf->Output("I"/*Définition du mode d'impression*/, /*Définition du nom du pdf*/'Liste_des_commandes_non_valides.pdf',/*Acceptation de l'encodage UTF8*/true);
?>