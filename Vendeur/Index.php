    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if (empty($_POST["Nom"])) {
        ...
        if (preg_match("/^[a-zA-Z]*$/", $_POST["Nom"])) {
          ...
          $ErNom = "<p>Evitez les caractères spéciaux SVP</p>";
        }
      ...
      if (!empty($Nom) AND !empty($Mot2Passe)) {
        ...
        $ChaineSelAgent = "SELECT * FROM `agent` WHERE `Nom_Agent` = '$Nom' AND `PasseAssoc` = '$Mot2Passe'";
        ...
        if (is_array($ResultaSelAgent)) {
          ...
          if (password_verify($Mot2Passe, $ResultaSelAgent["MotDePasse"])) {
            if ($ResultaSelAgent["Etat"] === "A") {
              $_SESSION['v'] = $ResultaSelAgent["PK_Agent"];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]=[];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]["nom"] = $ResultaSelAgent["Nom_Agent"];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]["post_nom"] = $ResultaSelAgent["PostNom_Agent"];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]["genre"] = $ResultaSelAgent["Genre_Agent"];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]["etat"] = $ResultaSelAgent["Etat"];

              $_SESSION['utilisateurs']["v_".$ResultaSelAgent["PK_Agent"]]["id"] = $ResultaSelAgent["PK_Agent"];

              header('location:gestion_produit.php');
            } else {
              $ErDeConnexion = "<p>Vendeur désactivé</p>";
            }
