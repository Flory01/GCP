<?php
    ...
    $pdf->AddPage();
        ...
        //Titre du tableau
        $Titre = utf8_decode("LISTE DES COMMANDES EN COURS DE LIVRAISON");
        ...
        $rq_sel_cmd = $connexion->prepare("SELECT * FROM `commande` AS `C` INNER JOIN `paiement` AS `P` ON P.commande = C.PK_Cmd INNER JOIN `temps` AS `T` ON T.PK_Temps = C.Date_Cmd WHERE P.Observation='NON PAYE' AND C.livraison='ENCOURS' AND C.Agent = $id_agent");
