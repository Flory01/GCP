...
<html lang="fr">
	...
	<body>
		...
		<?php
			...
			if (!empty($Nom) AND !empty($PostNom) AND !empty($Genre) AND !empty($MotDePasse) AND !empty($NumTelAgent) AND !empty($MDPC) AND !empty($NomPhoto)) {
							
				$PasseDef = password_hash($MotDePasse, PASSWORD_DEFAULT);
				$Etat = $_POST['Etat'];
				$RequeteInsertion = $connexion->prepare("INSERT INTO `Agent` SELECT NULL, :NomAgent, :PostNomAgent, :NumTelAgent, :GenreAgent, :MotDePasse, :PasseAssoc, :Etat FROM DUAL WHERE NOT EXISTS (SELECT * FROM `agent` WHERE `Nom_Agent` = '$Nom' AND `PostNom_Agent` = '$PostNom' AND `Genre_Agent` = '$Genre')");
				$RequeteInsertion->BindParam(':NomAgent',$Nom);
				$RequeteInsertion->BindParam(':PostNomAgent',$PostNom);
				$RequeteInsertion->BindParam(':GenreAgent',$Genre);
				$RequeteInsertion->BindParam(':MotDePasse',$PasseDef);
				$RequeteInsertion->BindParam(':PasseAssoc',$MotDePasse);
				$RequeteInsertion->BindParam(':NumTelAgent',$NumTelAgent);
				$RequeteInsertion->BindParam(':Etat',$Etat);
				$RequeteInsertion->execute();

				$RequeteSelctionAgent = $connexion->prepare("SELECT * FROM `agent` WHERE `Nom_Agent` = '$Nom' AND `PostNom_Agent` = '$PostNom' AND `Genre_Agent` = '$Genre'");
				$RequeteSelctionAgent->execute();
				$ResultatSelectionAgent = $RequeteSelctionAgent->fetch(PDO::FETCH_ASSOC);
									echo "nom agent : ".$IdAgent = $ResultatSelectionAgent["PK_Agent"];

				$Requete_photo = $connexion->prepare("INSERT INTO `photo_agent` VALUES (NULL,:Agent, :Nom, :Source)");
				$Requete_photo->BindParam(":Agent", $IdAgent);
		...
		<div style="display: flex;justify-content: center;margin-top: 100px;">	
			<form id="FormConnexion" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>"; method="post" enctype="multipart/form-data">
				...
				<input type="file" name="Photo" id="Photo" value="" multiple="" style="font-size: 16px">
				<?php echo($ErrPhoto); ?>
