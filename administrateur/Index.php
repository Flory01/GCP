  ...
  <?php 
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
      ...
      if (!empty($Nom) AND !empty($Mot2Passe)) {
        ...
        $ChaineSelAgent = "SELECT * FROM `administrateur` WHERE `Nom` = '$Nom' AND `PasseAssocie` = '$Mot2Passe'";
        ...
        if (is_array($ResultaSelAgent)) {
          ...
          if (password_verify($Mot2Passe, $ResultaSelAgent["MotDePasse"])) {
              ...
              $_SESSION['admin'] = $ResultaSelAgent["IdAdmin"];

              $_SESSION['utilisateurs']["admin_".$ResultaSelAgent["IdAdmin"]]=[];

              $_SESSION['utilisateurs']["admin_".$ResultaSelAgent["IdAdmin"]]["nom"] = $ResultaSelAgent["Nom"];

              $_SESSION['utilisateurs']["admin_".$ResultaSelAgent["IdAdmin"]]["id"] = $ResultaSelAgent["IdAdmin"];
